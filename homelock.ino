...
unsigned long TimerBlock;
unsigned long TimerMail;
bool block = false;
...
<head>\n\
  ...
  <link rel=\"stylesheet\" href=\"//use.fontawesome.com/releases/v5.3.1/css/regular.css\" integrity=\"sha384-ZlNfXjxAqKFWCwMwQFGhmMh3i89dWDnaFU2/VZg9CvsMGA7hXHQsPIqS+JIAmgEq\" crossorigin=\"anonymous\">\n\
...
void avisar(bool final = false) {
  String mensagem = "Sistema bloqueado por 30 minutos.";
  if(final) mensagem = "Sistema bloqueado definitivamente.";
  
  if(gsender->Subject("Home Lock")->Send("dactyllo@gmail.com", mensagem)) {
...
void loop() {
  if(block) {
    if(millis()-TimerMail >= 180000UL) {
      if(checkMail()) panico();
      TimerMail = millis();
    }
    if(millis()-TimerBlock >= 1800000UL) block = false;
  }
  server.handleClient();
}
